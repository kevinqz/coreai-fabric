"""Technique smoke: graph-split chaining (RFC F16 / playbook T3).

WHAT THIS EXERCISES: the 2-stage split mechanics — one asset, two entrypoints
(stage_a/stage_b), host chains A->B. Tiny random config. Checks the chained
output == monolithic torch fp16 (same weights) so the split wiring
(residual/cond/cos/sin hand-off) is correct. This is the lowering check the
/reflect loop runs after touching the graph-split technique.

PRECONDITION (playbook T3 / RFC F11): graph-split is valid only under a BOUNDED,
small host-loop pass count (4-10 denoise steps). An AR generator that reloads N
multi-GB programs per token passes cosine while being UNDEPLOYABLE — that is not
what this smoke validates.

EXPLICIT EXCLUSION — a green smoke is NOT a loadability guarantee (RFC F12/F16):
the split mechanism is checked at NL=4 layers; it cannot see the ANE 0x10004
ceiling that blocks a full 36-layer program. Do not infer full-model
loadability from this green.

REQUIRES: coreai_torch + torch + lingbotvla source. Skips with a clear message
otherwise."""
...
try:
    ...
    print(f"SKIP: graph-split smoke needs the convert toolchain + lingbotvla "
...
aim = ROOT / "build" / "_splsmoke" / "s.aimodel"
